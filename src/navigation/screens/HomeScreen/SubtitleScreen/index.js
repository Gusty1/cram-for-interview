...
const disposeSort = (subtitles, sortList) => {
  let ary;
  try {
    ary = JSON.parse(sortList[0].subtitleJsonAry);
  } catch (e) {
    // 排序資料損毀時回退至字母排序，避免整頁白屏
    console.error("disposeSort: 排序資料解析失敗，回退至字母排序", e);
    return subtitles.sort((a, b) => a.en_name.localeCompare(b.en_name));
  }
  return subtitles.map((item) => {
    const foundObj = ary.find((obj) => obj.subtitle === item.en_name);
    return foundObj
      ? { ...foundObj, ...item, sort: foundObj.sort }
      : { ...item, subtitle: item.en_name, sort: 99 };
  });
};
...
const SubtitleScreen = ({ navigation, route }) => {
  ...
  const getSubtitleList = useCallback(
    async (forceRefresh = false) => {
      try {
        ...
        const questionsMap = await getCachedQuestionsBatch(
          subtitleENList,
          forceRefresh,
        );
        ...
        if (sortList && sortList.length > 0)
          subtitles = disposeSort(subtitles, sortList);
  ...
  useEffect(() => {
    if (originSubtitleList.length === 0) return;
    // 以 originSubtitleList 為基準重新套用收藏狀態，避免 subtitleList 作為依賴導致無限迴圈
    const addFavSubtitleList = originSubtitleList.map((item) => ({
      ...item,
  ...
  }, [favoriteList, originSubtitleList, favoriteIdSet]);
      ...
      if (
        ...
        setSubtitleList(
          originSubtitleList.filter((item) => {
            if (
              showSubtitleItems.length > 0 &&
              changeSubtitleItems.some(
                (x) => x.en_name === item.en_name && x.selected,
              )
            ) {
              return true;
            } else {
  ...
  const onReordered = (fromIndex, toIndex) => {
    ...
    if (
      fromIndex < 0 ||
      fromIndex > subtitleList.length - 1 ||
      toIndex < 0 ||
      toIndex > subtitleList.length - 1
    )
      return;
    ...
    if (subtitleList && subtitleList.length > 0) {
      insertSubtitleSort(
        subjectEN,
        JSON.stringify(
          changeSubtileList.map((item, index) => {
            return {
